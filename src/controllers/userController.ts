...
export const updateProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 1. Validar unicidad del nuevo Nick (si cambia)
    if (nick && nick.trim() !== user.nick) {
      const cleanNick = nick.trim();
      const existingNick = await userRepository.findOne({ where: { nick: cleanNick } });
      if (existingNick) {
        res.status(400).json({ message: 'El nick ya está en uso por otro jugador.' });
        return;
      }
      user.nick = cleanNick;
    }

    // 2. Validar unicidad del nuevo Email (si cambia)
    if (email && email.trim() !== user.email) {
      const cleanEmail = email.trim().toLowerCase();
      const existingEmail = await userRepository.findOne({ where: { email: cleanEmail } });
      if (existingEmail) {
        res.status(400).json({ message: 'El correo ya está en uso por otra cuenta.' });
        return;
      }
    ...
    // 4. (Opcional) Si en GameHistory guardas el nick directamente, actualizarlo para no perder historial
    ...
    console.log(`👤 Perfil actualizado para usuario ID ${userId} (${user.nick})`);
