  ...
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      try {
        ...
        console.log(
          "⚠️ Token inválido en optionalAuth, continuando como invitado",
        );
      }
    }
...
export const requireMinElo = (minElo: number) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const userElo = req.userElo ?? 1200;
 ...
 */
export const getAuthenticatedUserId = (req: Request): number | undefined => {
  return req.userId
};
